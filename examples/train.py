...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="mlp",
        choices=["cnn", "mlp", "vit"],
        help="What model to use. [cnn,mlp,vit]",
    )
    parser.add_argument(
        "--batch_norm",
        action="store_true",
        default=False,
        help="Whether to use BN in model.",
    )
    ...
    parser.add_argument(
        "--sgd_momentum", type=float, default=0.0, help="For SGD: momentum value to use"
    )
    parser.add_argument(
        "--sgd_nesterov",
        action="store_true",
        default=False,
        help="For SGD: use Nesterov momentum",
    )
    parser.add_argument(
        "--sgd_dampening",
        type=float,
        default=0.0,
        help="For SGD: dampening value to use",
    )
    parser.add_argument(
        "--adam_betas",
        type=float,
...
def start_training(args):
    ...
    for epoch in range(epoch, args.epochs):
        train_single_epoch(
            epoch, args.batch_size, criterion, model, optimizer, train_dataset
        )
...
def train_single_epoch(
    ...
    for imgs, lbls in (
        pbar := tqdm(
            train_dataset.as_batched(batch_size),
            total=total_batches,
            desc=f"Epoch [{epoch}]",
        )
    ):
...
def evaluate(epoch: int, batch_size: int, model: nn.Module, test_dataset: data.MNIST):
    ...
    for imgs, lbls in tqdm(
        test_dataset.as_batched(batch_size),
        total=total_batches,
...
def load_checkpoint(
    checkpoint_file: Path, model: nn.Module, optimizer: optim.Optimizer
):
    ...
    with np.load(str(checkpoint_file)) as d:
        ...
        i = len("optimizer.")
        optimizer_dict = {
            key[i:]: d[key] for key in d.keys() if key.startswith("optimizer.")
        }
...
def get_model(args):
    ...
    if model_name == "cnn":
        ...
        model = models.MLP(
            sizes=[28 * 28, 128, 64, 10], flatten_first=True, norm_layer=norm_layer
        )
    elif model_name == "vit":
        model = models.ViT(
            img_size=28,
